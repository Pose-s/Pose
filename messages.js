...
// ===== Fotocamera interna con disegno e testo =====
...
const cameraEditToolbar = document.getElementById('cameraEditToolbar');
const cameraEditBottom = document.getElementById('cameraEditBottom');
...
const cameraDiscardBtn = document.getElementById('cameraDiscardBtn');
...
photoRightBtn.addEventListener('click', openCamera);

async function openCamera() {
  cameraModal.classList.remove('hidden');
  cameraCanvas.classList.add('hidden');
  cameraVideo.classList.remove('hidden');
  cameraCaptureBar.classList.remove('hidden');
  cameraEditToolbar.classList.add('hidden');
  cameraEditBottom.classList.add('hidden');
  currentTool = null;
  ...
  try {
    ...
    console.error('Fotocamera non disponibile:', error);
...
cameraCaptureBtn.addEventListener('click', () => {
  ...
  camCtx.drawImage(cameraVideo, 0, 0, w, h);
  ...
  cameraCaptureBar.classList.add('hidden');
  cameraEditToolbar.classList.remove('hidden');
  cameraEditBottom.classList.remove('hidden');

  undoStack = [cameraCanvas.toDataURL()];
});
...
function saveUndoState() {
  undoStack.push(cameraCanvas.toDataURL());
...
undoEditBtn.addEventListener('click', () => {
  ...
  const img = new Image();
  img.onload = () => camCtx.drawImage(img, 0, 0, cameraCanvas.width, cameraCanvas.height);
  img.src = prev;
});
...
function getCanvasPos(e) {
  ...
  const scaleX = cameraCanvas.width / rect.width;
  const scaleY = cameraCanvas.height / rect.height;
...
function startDraw(e) {
  ...
  camCtx.beginPath();
  camCtx.moveTo(pos.x, pos.y);
}

function moveDraw(e) {
  if (!isDrawing || currentTool !== 'draw') return;
  const pos = getCanvasPos(e);
  camCtx.lineTo(pos.x, pos.y);
  camCtx.strokeStyle = currentColor;
  camCtx.lineWidth = 6;
...
cameraCanvas.addEventListener('mousedown', startDraw);
cameraCanvas.addEventListener('mousemove', moveDraw);
cameraCanvas.addEventListener('mouseup', endDraw);
cameraCanvas.addEventListener('touchstart', (e) => startDraw(e), { passive: true });
cameraCanvas.addEventListener('touchmove', (e) => moveDraw(e), { passive: true });
cameraCanvas.addEventListener('touchend', endDraw);
...
cameraCanvas.addEventListener('click', (e) => {
  ...
  cameraTextInput.style.left = `${pos.clientX}px`;
  cameraTextInput.style.top = `${pos.clientY}px`;
  cameraTextInput.style.color = currentColor;
  cameraTextInput.classList.remove('hidden');
  cameraTextInput.value = '';
  cameraTextInput.focus();
});

cameraTextInput.addEventListener('keydown', (e) => {
  if (e.key === 'Enter' && pendingTextPos) {
    camCtx.fillStyle = currentColor;
    camCtx.font = 'bold 36px Inter, sans-serif';
    camCtx.fillText(cameraTextInput.value, pendingTextPos.x, pendingTextPos.y);
    cameraTextInput.classList.add('hidden');
    pendingTextPos = null;
    saveUndoState();
  }
});

cameraDiscardBtn.addEventListener('click', closeCamera);
...
cameraSendBtn.addEventListener('click', async () => {
  ...
  try {
    ...
    console.error('Errore invio foto fotocamera:', error);
  }
});
